  ...
  return (
    <div className="grid gap-3">
      <label className="text-sm text-[var(--text-muted)]">
        Metodo de pago
        <select className="mt-2 w-full rounded-lg border border-[var(--border-soft)] bg-black px-3 py-2 text-white" value={method} onChange={(event) => onMethodChange(event.target.value as PaymentMethod)}>
      ...
      {method === "mixto" ? (
        <div className="grid gap-2">
          {splits.map((split, index) => (
            <div key={index} className="grid gap-2 rounded-lg border border-[var(--border-soft)] bg-black/25 p-2 md:grid-cols-[1fr_120px_1fr_auto]">
              <select className="rounded-lg border border-[var(--border-soft)] bg-black px-3 py-2 text-white" value={split.method} onChange={(event) => update(index, { method: event.target.value as PaymentSplit["method"] })}>
                {methods.map((item) => <option key={item} value={item}>{item}</option>)}
              </select>
              <MoneyInput value={split.amount} onChange={(event) => update(index, { amount: Number(event.target.value) })} />
              <input className="rounded-lg border border-[var(--border-soft)] bg-black px-3 py-2 text-white" placeholder="Referencia" value={split.reference ?? ""} onChange={(event) => update(index, { reference: event.target.value })} />
